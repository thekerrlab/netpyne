...
Version: 2014jun10 by Cliff Kerr (cliffk@neurosim.downstate.edu)
...
BREAKPOINT {
  ...
  if (verbose>1) { : Verbose turned on?
    ...
    fprintf(outfile,"t=%08.2f || cell=%05.0f || delta=%8.2f || gAMPA=%8.2f || gNMDA=%8.2f || gGABAA=%8.2f || gGABAB=%8.2f || factor=%8.2f || I=%8.2f || V=%8.2f || u=%8.2f (timestep)\n",t,cellid,delta,gAMPA,gNMDA,gGABAA,gGABAB,factor,I,V,u);
...
NET_RECEIVE (wAMPA, wNMDA, wGABAA, wGABAB) {  
  ...
  if (verbose>0) { : Verbose turned on?
    ...
    FILE *outfile; // Declare file object
    outfile=fopen(filename,"a"); // Open file for appending
    fprintf(outfile,"t=%08.2f || cell=%05.0f || flag=%1.0f || gAMPA=%8.2f || gNMDA=%8.2f || gGABAA=%8.2f || gGABAB=%8.2f || V=%8.2f || u=%8.2f (event)\n",t,cellid,eventflag,gAMPA,gNMDA,gGABAA,gGABAB,V,u);
    fclose(outfile); // Close file
    ENDVERBATIM
  }
